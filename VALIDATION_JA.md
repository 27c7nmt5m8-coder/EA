...
実施日: 2026-09-13（Strategy Tester接続判定修正）

**修正ソースの模擬実行522チェック（既存500＋今回22）、JSON回帰55チェック、静的監査495チェック、CSV集計20チェックが通過しました。MetaEditorネイティブコンパイル未実測、実機MT5バックテスト未実測です。実際のAI API通信も未実測で、EX5は含みません。**

件数には初期化、方向違い、倍率違い、同一試験内の複数確認を含みます。522件の独立した相場シナリオや、収益性の検証という意味ではありません。

## Strategy Tester接続判定修正の基点・根本原因

基点はGitHub `27c7nmt5m8-coder/EA` の最新main `701e2eedc54a791d9643dc928065875b0929a2ee` です。全42追跡ファイルをGit blob SHAで照合し、修正前に既存テスト一式（模擬500、JSON55、静的495、集計20）の通過を確認しました。前回の未統合PR #3のロック修正とは独立した変更です。

`EntryPreflight()` が `TERMINAL_CONNECTED` をテスターでも必須としていたため、`MQL_TESTER=true` かつ接続なしの状態では、他の条件が正常でも初期化後のエントリーを拒否していました。追加回帰を先に実行すると、未修正コードは `FAIL: tester disconnected preflight permits eligible entry` で失敗しました。

製品コードの変更は、この拒否条件に `!MQLInfoInteger(MQL_TESTER)` を加えた1行のみです。実運用時の接続確認と、後続の4つの売買許可、履歴・リスク等の判定はそのままです。未解決注文を手動照合する `ReconcilePendingOrder()` 内の接続確認も変更していません。全13製品ソースを比較し、今回の条件だけを逆変換するとmainの全バイトと一致することを確認しました（`verification/tester_connection_scope.json`）。

mockの `MQLInfoInteger(MQL_TESTER)` と `TerminalInfoInteger(TERMINAL_CONNECTED)` は、既存の `tester` / `connected` で独立にtrue/falseを返せます。この不具合の再現に必要な仕様差はなく、mock実装の変更は不要でした。既存テストでは `connected=true` が既定値で、テスターのケースは主にOnInit成功だけを確認していたため、未接続での発注経路が未検証でした。

## 追加回帰22チェック

| 条件・経路 | 修正後の結果 |
| --- | --- |
| 実運用＋接続あり／なし | 適格候補を許可／拒否 |
| テスター＋接続あり／なし | 他条件が正常ならどちらも許可 |
| 実運用／テスターで売買許可OFF | どちらも拒否 |
| 未接続テスターのOnInit→OnTimer | 合成価格の適格候補から模擬注文1件・ポジション1件 |
| 同一M1でOnTimerを再呼出し | 追加注文なし |
| 未接続の実運用→再接続 | 接続前は注文0、接続後は適格候補を発注 |
| 未接続テスター＋総リスク上限超過 | 注文0、ポートフォリオ拒否数1 |
| 未接続テスター＋未解決注文 | 新規拒否 |

ケースは `tests/scenarios.cpp` の `tester_connection_tests()` に追加し、`python3 tests/run_all.py` から既存ケースと一緒に実行します。旧監査の基準ハッシュは変更せず、今回の接続条件だけを厳密に旧形式へ戻して照合しています。条件を丸ごと除外したり、関数全体を監査対象外にしたりしていません。

**MetaEditorネイティブコンパイル未実測、実機MT5バックテスト未実測です。** 同一期間・銘柄・入力値のMT5テスターで、初期化後のログ、取引数、他の拒否理由、実際の売買結果を確認する必要があります。今回の模擬再現は、実データで取引数0になるすべての原因を除外するものではありません。
...
初回開発時にv2.43の326チェックへ174チェックを追加しました。今回はその既存500（v2.42由来207＋v2.43追加119＋v2.44追加174）を維持してすべて再実行し、接続判定の22チェックを追加しています。旧版の試験ケースと共通reset処理は変更していません。
...
| 区分 | 結果 | 内容 |
| --- | --- | --- |
| 本体・Worker模擬実行 | 522 / 522 | 既存500＋今回22 |
...
- v2.43エンジン171関数のうち、初回v2.44で変更した11関数以外の160関数を照合。今回のEntryPreflightの接続条件だけを厳密に戻すと、160関数が旧SHA-256と一致する。残りのエントリー・保護条件は除外せずに比較する。
- v2.42由来162関数の比較も保持。従来の6関数の分析フックに加え、今回のEntryPreflightのテスター接続例外だけを厳密に戻して旧ハッシュへ照合し、元の保護計算・条件・注文引数の一致を確認。
...
個別には `verify_v244.py`、`verify_json_regression.py`、`verify_stats.py`、`audit_source.py` を実行できます。旧 `verify_v242.py` / `verify_v243.py` は互換エントリーとして全522模擬チェックを実行します。
