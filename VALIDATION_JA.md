...
- 失敗は実装不具合・テスト不具合・mock不整合・環境依存・既知の制限に切り分け、根拠を記録します。ツール不足時も可能な静的検証・Pythonテスト・コードレビューを続け、残るmock等はCIで補います。
- MQL5変更では利用可能なMetaEditorで本体・Workerをネイティブコンパイルし、目標は `0 errors / 0 warnings` とします。関連するStrategy Tester・デモ実機確認は [実機確認項目](#実機で残る確認とv243比較) を変更範囲に応じて選びます。文書変更だけで一律に実機試験を要求しません。
- Testerと実運用の差、再初期化・再起動・再接続、Global Variable・mutex・owner lockの実MQL5 semanticsを考慮します。警告は原因を調べ、依頼範囲内で最小修正します。
- MetaEditor/MT5が利用できないこと自体を失敗扱いしません。未実施なら「ネイティブコンパイル未実測」と明記し、過去の記録と今回の測定を区別します。
- 検証生成物（`verification/*.cpp`、`integration` / `json_test` とその `.exe`、`case.json` / `initial.json`、`__pycache__/`）をコミットしません。配布結果の更新時は実行結果と対象ソースSHAを対応させます。
