...
- `ReadIndicator` の1行でデータ要求を先に実行する。ハンドル、取得件数、計算済みバー数、非数・EMPTY_VALUEの拒否は保持し、スコア・売買条件・リスク・注文処理・入力値は変更しない。
- mockにテスターの要求時計算を反映。回帰テストの修正前失敗と修正後成功を確認し、全mock582・JSON55・CSV20・静的495・13ソース照合が通過。従来の基準ハッシュを変更せず、今回の1行の順序変更だけを厳密に逆変換する。
- 根拠: [MQL5のテスター指標計算仕様](https://www.mql5.com/en/docs/runtime/testing)。MetaEditor 6182では本体・Workerとも0 errors / 0 warnings。
- 同じ2026-06-01〜2026-09-01のUSDJPY/M1実ティック試験で指標待機の解消を確認。ただし取引は前後とも0件で、後段の価格・コスト・SL形状等のgateの確認は残る。売買条件の緩和は行わない。
