...
- GitHub main `701e2eedc54a791d9643dc928065875b0929a2ee` を基点とし、`EntryPreflight()` の接続拒否条件1行だけを修正。
- `!TERMINAL_CONNECTED` による拒否を `!MQL_TESTER && !TERMINAL_CONNECTED` とし、Strategy Tester時だけライブ接続を不要にした。実運用の未接続拒否、売買許可、注文不確実性、総リスク、スコア、SL/TP、ロット計算は維持。
- `tests/scenarios.cpp` に22チェックを追加。テスター／実運用×接続あり／なし、売買許可OFF、テスターのAUTO発注、同一M1の重複防止、実運用での再接続後発注、総リスク上限、未解決注文停止を確認。
- 接続関連mockは既存の `tester` / `connected` / `permissions` で必要な状態を独立に再現できるため実装変更なし。旧テストの接続既定値trueと、テスターで初期化だけを確認していたカバレッジ不足を追加ケースで補った。
- 過去の静的監査ハッシュは変更せず、今回の接続条件だけを厳密に旧条件へ戻して比較する。既存テストは削除・弱体化していない。
- 全テスト：模擬522（既存500＋今回22）、JSON55、静的監査495、集計20が通過。全13製品ソースの比較で上記1条件以外の差分がないことを確認。
- **MetaEditorネイティブコンパイル未実測。実機MT5バックテスト未実測。** 実ブローカー・AI/API通信も未実測。
- バージョンは2.44。未統合のロック修正PR #3はこの変更に含めていない。
