# MTFAutoTrader v2.45 — 日本語説明書

更新日: 2026-09-21

## v2.45：MANUALのWAIT理由と自動トレンドライン表示

GitHub main `bcb41c8f9f775b170154ecddebeb0f6624d947ca` を基点にしています。過去のv2.45 ZIP・PR #11の診断変更は取り込んでいません。

- MANUALパネルはM1 ATR・履歴・quote待ち中も表示します。BUY/SELLのWAITはSLの向き・距離、M1 ATR、spread、ロット・margin、売買許可、未解決注文、MC/DD、Portfolio Risk、M1バー、方向、注文ロック等の実際の判定理由を表示します。ヘッダーとダッシュボードもプレビューの可否に合わせます。
- Readyはその時点の**注文プレビュー**です。クリック後も従来の共通注文処理が最新価格・注文チェック・mutex等を再確認するため、約定を保証しません。既存ポジションのSL・建値移動・トレーリングは従来どおりです。
- 既定値は `MaxSpreadATR=0.10`、`MaxSpreadSL=0.15`、`MaxSpreadPoints=0` のままです。SLを遠ざけても、spreadがM1 ATRの10%を超える場合などはWAITが続きます。従来はこの失敗もSLエラーと誤表示していました。パネル下部のBid/Ask・spread・ATR・要求SL/丸め後SL・broker制約も確認してください。
- `ShowAutoTrendLines=false` を追加しました。`EnableAutoTrendLines=true` のまま描画だけをOFFにできます。内部の仮想トレンド・LineScore・注文判定は変わりません。水平線、MANUAL SL、TPプレビュー、ボタン、ダッシュボードは対象外です。以前の描画が必要なら `ShowAutoTrendLines=true` にします。

本体のバージョンは2.45です。導入先・既存スクリプトとの互換性のため、本体ファイル名は `MTFAutoTrader_3Mode_AI_v2_44.mq5` を維持します。Workerは機能変更なしで2.44、通信プロトコル242・JSON schemaも維持します。srcの13ファイルを一体で入れ替えてコンパイルしてください。**バージョン表示と検証対象SHAで確認し、ファイル名だけで旧版と判断しないでください。**

本体・Workerのネイティブ結果と回帰テストは [v2.45検証](VALIDATION_JA.md#v245) を参照してください。ユーザーのEURUSD実機で発生した瞬間のATR・spreadは未採取のため、実機の唯一の阻害要因はまだ確定していません。

以下のv2.44修正・検証記述は過去の記録です。現版の検証結果は上記v2.45節を優先してください。
...
**このパッケージはソースコード版です。v2.45のネイティブ結果は検証報告を参照してください。実機のMANUAL操作・勝率・取引頻度の比較は未実測です。** 同梱の模擬検証は、実市場での成績を示すものではありません。検証範囲は `VALIDATION_JA.md` を参照してください。
