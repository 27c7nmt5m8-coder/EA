...
更新日: 2026-09-13（v2.44 Strategy Tester接続判定修正）
...
v2.44の接続判定修正版では、`EntryPreflight()` のライブ接続チェック `TERMINAL_CONNECTED` を `MQL_TESTER` 時だけ除外します。これにより、ライブ接続がないテスターでも他の発注条件を満たす候補が発注処理へ進めます。実運用時の接続確認、テスターを含む売買許可・未解決注文・総リスク上限などの安全判定は維持しています。売買ロジック、スコア、SL/TP、ロット計算、入力値は変更していません。

この修正はGitHub main `701e2eedc54a791d9643dc928065875b0929a2ee` を基準とする独立した変更です。修正前の「テスター＋未接続」での拒否を模擬再現し、修正後は既存500＋追加22チェックが通過しました。**MetaEditorネイティブコンパイル未実測、実機MT5バックテスト未実測です。** 実データで必ず取引が発生することや収益性を確認した結果ではありません。
